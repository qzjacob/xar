...
#   OLLAMA_API_KEY=ollama            可选占位；配合 XAR_GLM_WORKER_LOCAL_FIRST=true +
#   XAR_GLM_WORKER_LOCAL_MODEL=...   让 glmworker 抽取走本地 ollama(零成本,云 GLM 自动回落)
...
| 嵌入 | fastembed（CPU，默认 bge-small 384d；中英混合语料经 `xar reembed` 升级至 e5-large 1024d，自动加 e5 `query:`/`passage:` 前缀） | TEI/vLLM 服务 |
...
- **本地优先抽取（2026-07-19 起）**：glmworker 的钉扎链前插**本地 ollama 模型**（`qwen3-14b-local` → RTX 3090 上的 `qwen3-14b-xar`，40 篇黄金集赛马胜者，零成本零延迟）；云 GLM 订阅降为**自动回落**（本地端点不可达/超时/空响应即轮转，`mlrun --exclusive` 训练独占期亦然）。开关 `XAR_GLM_WORKER_LOCAL_FIRST` + `XAR_GLM_WORKER_LOCAL_MODEL`（换代/回滚改 env 即可）；模型评测 runbook `scripts/bench_local_llm.py`。详见 `DESIGN.md §6.1`。
- **Claude Max 订阅执行器（第二执行路径，peer/fallback）**：`ModelSpec` 新增 `executor` 字段（`litellm` 默认 | `agent_sdk`）——两个订阅条目 `claude-opus-max`/`claude-sonnet-max`（`Billing.SUBSCRIPTION`、0/0 价、`litellm_model="anthropic-max/…"` 避与 token 版索引撞车）经 **Claude Agent SDK** 用 **Max 订阅**（OAuth 登录 `~/.claude/.credentials.json`）跑**单次补全**、**零 token 计费**——与 GLM 订阅池同一「订阅内白嫖、订阅外分文不花」纪律。订阅强制：在**剥掉 `ANTHROPIC_API_KEY` 的子进程**里**按文件路径**调用 worker（`models/_agentsdk_worker.py`，`allowed_tools=[]`/`max_turns=1`/`bypassPermissions`、零 xar 依赖），父进程 `os.environ` 不动、并发安全；`agentsdk.available()` 闸 `anthropic_max_enabled` + `claude_agent_sdk` 可导入 + `claude` CLI 在 PATH + 凭证在位 → **仅宿主机**，docker 缺任一即跳过、回退 GLM/DeepSeek。仅进 **STRONG 链（editor/synth/debate）作 token+GLM lead 之后的候选**（`prefer_billing=TOKEN` 保 token 优先）、**不进批量**（KG_EXTRACT/EXPERT/WECHAT_TRIAGE 仍钉 GLM——~6.5s/次 + 订阅额度，不适合千级文档）；`complete_stream()` 跳过（单次、不支持工具流）。便捷钉扎链 `llm.CLAUDE_MAX_PIN=("claude-opus-max","glm-5.2-sub","glm-4.6-sub")` 强制高价值流程走 Opus、订阅/宿主不可用则优雅降级 GLM。config `anthropic_max_enabled`(默认 True)/`anthropic_max_model`(claude-opus-4-8)/`anthropic_max_effort`(high)/`anthropic_max_timeout_s`(180) + `pyproject [agentsdk]` extra；`/api/ops/llm` 增 `anthropicMax` 块；**仅宿主侧部署**（凭证不进 docker）。
...
`complete()` / `complete_json()` 接受 `task=`；后者走 JSON-mode。providers 含 deepseek / anthropic / openai / **zhipu(=GLM)** / **moonshot(=Kimi)** / **ollama(本地,仅钉扎 spec)**，env key 分别为 `GLM_API_KEY`（亦认 `ZHIPU_API_KEY`）/ `MOONSHOT_API_KEY`（亦认 `KIMI_API_KEY`）/ `OLLAMA_API_KEY`（占位即可），均可选。每次运行有美元预算上限（`XAR_LLM_MAX_USD_PER_RUN`，默认 5）。
...
xar glm-worker <cmd> 语义抽取常驻工人(Fetchy)：run [--once] · status · probe——钉扎链本地优先(ollama qwen3-14b-xar 零成本,云 GLM 订阅自动回落;额度耗尽自动等待、恢复自动重启)；持续拉取语义源 + 10 年历史回填（compose 服务 glmworker）
xar wechat-mine     微信抽取前 SNR triage：--once 打一批分 / --stats 看保留率（与抽取同钉扎链:本地优先、云订阅回落；仅高分文档进深度抽取）
xar wechat-account <cmd> 策展公众号名册：add · list · rm      xar wechat-targets 当前挖掘目标（被挑战论点优先）+ 中文猎词
xar futu [id]       富途 OpenD：拉快照+资讯+板块 / 状态 + 本体缺口（--gaps；需 XAR_ENABLE_FUTU + OpenD）
xar reembed         全库重嵌入到中英模型（e5-large 1024d，升级中文检索）；完成后设 XAR_EMBED_MODEL/XAR_EMBED_DIM 重启
...
### 微信多层级挖掘系统（多层级智能挖掘，`mining/` + `ontology/cn_routing.py`）

公众号信噪比极低：旧管线对**每篇**微信文章无差别发**两次满额 GLM 调用**（`build_kg` + `expert`），SNR 判断在昂贵调用**内部**、事后才丢弃——实测保留率仅 **3.75%**，约 **96% 的 GLM 额度烧在噪音上**。新系统在深度抽取**之前**插入一层廉价、GLM 订阅钉扎的 **SNR triage 闸**，只让高信噪比文章消耗深度额度，形成 **T0→T4 分层**：

- **T0 目标化**（`mining/targeting.py`，零 LLM）——从被信号/事件**挑战的活跃论点**反推「该猎什么」：被挑战公司优先，带中文别名 + 主题/路线中文猎词 + 论点 `watch_event_types`。
- **T1 策展采集**（`mining/roster.py` + `wechat_accounts` 表）——运营方在 we-mp-rss UI 订阅垂直号后登记 `feed_id→主题/公司/tier`，`glm-worker` 逐号拉取（**名册空则退回聚合 `/rss`**）；**刻意不做公众号关键词搜索**（不可靠、且会触发限流封号）。
- **T2 抽取前 SNR 闸**（`mining/triage.py`）——① **零 LLM 确定性预筛**：中文路由/别名/可解析公司**全不命中 → 直接打噪音地板分 0.03、跳过 LLM**（免费滤掉盲目名册的闲聊）；② 幸存者走**一次** GLM 钉扎的 `WECHAT_TRIAGE` 短调用，注入命中主题/路线 + **已知 KG 摘要**（新颖度对照）+ 公司论点 `watch_event_types`（支柱命中）；③ **可审计融合**（`0.35·priority + 0.25·credibility·¬小作文 + 0.20·novelty + 0.20·specificity`）+ **小作文地板** + **novelty·specificity 救回**（补微信无阅读数、救回冷门号扎实一手信息）→ 写 `documents.triage_score`；ingest 期未链公司的按 triage 结果**回填** `company_id`。
- **T3/T4 深度抽取**（`kg/extract.build_kg` + `kg/expert.process`）——即上述两次 GLM 调用，现由**两条 NULL 安全 WHERE 守卫**（`mining.triage.wechat_pending_clause`）按 `triage_score >= wechat_deep_min`（默认 **0.4**）放行；**未 triage（NULL）照旧全流、非微信短路完全不受影响**（加性、向后兼容）。

**订阅纪律**：triage 是新 `WECHAT_TRIAGE` 任务类（`CHEAP_BULK` + 订阅制优先、**从不走计量 token**；2026-07-19 起钉扎链首为本地 `qwen3-14b-local`，云 GLM 订阅回落）；`glm_worker._llm_stage` 在 `llm.pinned(pin)` 内**先 triage 后 build_kg**（同周期新拉的微信文档先打分再决定是否深抽）。**中文路由表**（`ontology/cn_routing.py`，code-as-truth）补齐了此前全英文关键词表的中文缺口：8 主题 + 33 tr_\* 的中文关键词，测试断言每个 key ∈ `registry.THEMES`/合法路线（镜像 `ROUTE_THEMES` 不变式，防与本体漂移）。

**配置/入口**：`wechat_miner_enabled`（关闭=退回旧的无差别抽取）、`wechat_deep_min=0.4`、`glm_worker_triage_docs=40`；schema 加性列 `documents.triage_score/triaged_at/triage(JSONB)` + 部分索引、`wechat_accounts` 表。CLI `xar wechat-mine [--once|--stats]` / `xar wechat-account {add,list,rm}` / `xar wechat-targets`；API `GET /api/ops/wechat-mining`。

**中文嵌入升级**：中英混合语料（中文公众号/cninfo + 英文 filing）的检索质量可经 `xar reembed` 全库重嵌到中英模型——fastembed 无 bge-m3，故升级路径为 `intfloat/multilingual-e5-large`（1024d），`embeddings.py` 自动加 e5 `query:`/`passage:` 不对称前缀；config 默认仍 bge-small（交钥匙），部署经 `.env`（`XAR_EMBED_MODEL`/`XAR_EMBED_DIM`）切换。

## 富途 OpenAPI 接入（Futu/moomoo → 本体 + 资金流）

富途补齐了 FMP/finnhub 缺失的 **港股 + A 股 + 美股（带中文名）广度**——估值、资金流、板块/产业链分类，喂同一条 parse→embed→triage→KG 管线与 alt-signal 论点修正引擎。

**本地网关姿态（镜像 Wind）**：`futu` Python SDK 只与本地 **OpenD 网关守护进程**（默认 `127.0.0.1:11111`）对话，OpenD 用账号登录、代 SDK 访问富途服务器；SDK 从不直连。**默认关**——`XAR_ENABLE_FUTU=true` + 可达 OpenD 才 arm（缺 OpenD/SDK 即 graceful-skip 返回空、不报错）。

三个数据面（`providers/futu.py`）：
- **市场快照 → 规范估值 + 价格**：`get_market_snapshot` 经 `FUTU_SNAPSHOT_MAP`（`ontology/standards.py`）把 PE/PB/市值/EPS/股息率归一到 `FinMetric`，落 `fundamentals(source=futu)` + 最新价格 bar 落 `prices`（按代码前缀定币种 HK→HKD / SH·SZ→CNY / US→USD）。
- **资讯 → documents**：`get_search_news` → `Doc(source=futu, doc_type=news, permission=grey)`，**仅标题 + 规范链接**（无正文可取，自用姿态），经 `cn_routing` 从中文标题打主题标（仅填空）。
- **板块 → 本体**：`get_owner_plate` → `futu_plates` 表（公司↔板块成员 + 映射主题）；板块名（中文）经 `ontology/futu_plates.py` **直接复用 `cn_routing`**（无新建映射表，天然继承 code-as-truth 守卫）映到 8 主题；`plate_theme_gaps()` = 纯 DB **本体缺口发现器**（富途板块暗示主题 vs 当前策展主题的差集，供 ops/复核）。
- **ticker↔富途代码**：`code_from_tickers`——HK 5 位零填充（`HK.00700`）、`SH.`/`SZ.`、`US.SYM`；优先 HK/CN（富途长板），回退 US。

**资金流 alt-signal**：`alt.futu_main_capital_flow`（`ontology/altdata.py`，company-scope、daily、`good_when=rising`、支柱 demand/valuation）——主力（超大单+大单）日度净流入，机构资金盘面足迹；经 `altdata._futu_code` 派生绑定到**任何带 HK/CN/US ticker 的公司（~436 家）**、无需策展；provider `providers/alt/futu_flow.py`（港股常无 `main_in_flow` → 回退 `super+big`），随既有 `alt.pull_all` 节律运行。

**Universe 扩容（`scripts/futu_universe_gen.py`，零 LLM，仅复核）**：从主题相关板块经 `cn_routing` 映射 → 取成员 → 按市值每主题 top-N、剔除已在册 → 写 `docs/futu/futu_universe_candidates.py` 供**人工复核**。**不自动并入 `COMPANIES`**——概念板块是大杂烩（"人工智能"含中国移动/顺丰），直接注入会污染本体；promote 是人工策展步骤。

```bash
# 1) 下载并运行 OpenD（本地网关），用富途账号登录（监听 127.0.0.1:11111）
pip install '.[futu]'                        # 或 pip install futu-api
export XAR_ENABLE_FUTU=true
# docker 内 OpenD 跑在宿主：FUTU_OPEND_HOST=host.docker.internal
xar futu 700                                 # 拉一家的快照+资讯+板块；或 API: GET /api/ops/futu
```

**接线**：`providers._MARKET`/`status()` 纳入 futu；`daily.py` 加 `futu` 源（`available()` gated）；`glm_worker` 每轮拉一个轮转切片的富途资讯；CLI `xar futu`；`/api/ops/futu`；config `futu_host/futu_port/enable_futu`；`pyproject` `[futu]` extra；`.env.example` 富途块。
...
- **人工审批**：报告默认 `awaiting_approval`，经人审才发布；强制非投资建议免责声明。**状态闸（2026-07-20）**：`/approve` 仅接受 `awaiting_approval`（`published` 幂等返回、`running/failed` 拒绝）；任何流水线异常统一落 `failed`，run 不再卡死 `running`。可选 `XAR_API_TOKEN`（默认关）为变更类 `/api/*` 端点加凭证护栏。
- **成本控制**：计价感知的任务路由（glmworker 抽取走**本地 ollama 零成本**、云 GLM/Kimi 订阅制封顶账单作回落、质量走 token 跨厂回退）+ 计费感知成本追踪 + 单次运行美元预算上限（`XAR_*` 可调）。**计量数据 API 同样封顶**：X (Twitter) 源默认关 + 全调用方共用 $20/月硬顶（`api_spend` 账本，fail-closed，`XAR_X_MONTHLY_BUDGET_USD` 可调）。
