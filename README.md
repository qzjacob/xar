...
xar glm-worker <cmd> GLM 订阅额度常驻工人：run [--once] · status · probe——额度耗尽自动等待、恢复自动重启；持续拉取语义源 + 10 年历史回填 + 钉扎 GLM 的语义抽取（compose 服务 glmworker）
xar wechat-mine     微信抽取前 SNR triage：--once 打一批分 / --stats 看保留率（GLM 钉扎、订阅计费；仅高分文档进深度抽取）
...
**GLM 订阅纪律**：triage 是新 `WECHAT_TRIAGE` 任务类（`CHEAP_BULK` + 订阅制优先，GLM 钉扎、**从不走计量 token**）；`glm_worker._llm_stage` 在 `llm.pinned(GLM_PIN)` 内**先 triage 后 build_kg**（同周期新拉的微信文档先打分再决定是否深抽）。**中文路由表**（`ontology/cn_routing.py`，code-as-truth）补齐了此前全英文关键词表的中文缺口：8 主题 + 33 tr_\* 的中文关键词，测试断言每个 key ∈ `registry.THEMES`/合法路线（镜像 `ROUTE_THEMES` 不变式，防与本体漂移）。
