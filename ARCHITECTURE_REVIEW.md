...
> 评审视角：技术 / 用户 / 架构 / 第一性原理。
> 决策前提（已与项目方确认）：**目标部署形态 = 自用 / 单团队研究工具**；平台现为**三个并列顶层模块**——产业链投研主轴（研究终端 **Genny** `/genny`）、运维控制台（Operations Console **Jarvy** `/jarvy/*`）、**前沿探索（Exploration Romy `/romy`）**。Exploration 已从"早期范围漂移信号"转正为正式第三模块，并经独立 agent 审计 PASS。（**2026-07-11 更名**：`/ops/*` 与 `/explore/*` 现仅为 302 兼容重定向，实际路由是 `/jarvy` 与 `/romy`；Jarvy 侧栏现 **11** 个子页——含新增的 monitor 与 fetchy；另有 `/phanny/*` 模块。）**前端现为四个并列 shell——默认首页 `/` 的对话式工具调用分析师 **Chathy**、宏观指标平台 **Andy** `/andy`（vendored `src/slx` siliconomics + 勾稽层，见演进 7）、研究终端 **Genny** `/genny`（原 Research Portal `/`；旧 `/segment/:id`·`/company/:id` 现 302 重定向到 `/genny`，App.tsx:53-67）、结构化票据/期权台 **Fenny** `/fenny`——四者共享同一数据+本体底座（详见演进 6/7）。**
...
**这是一份品味明显高于行业平均的工程实现**：~10,860 行代码撑起了 15+ 数据源（含新增 arXiv / journals 两个 frontier 源 + Finnhub 公司新闻；`providers.status()` 现为 **14** 个 provider，其中 **FMP 已被上游永久下线**，见下）+ 双时态图谱 + **时间锚定的语义数据库** + 多 Agent 流水线 + **Dagster 日度自动 ingest** + 全栈四模块终端，"交钥匙 + 优雅降级"的工程纪律扎实，许可合规作为 CI 硬规则体现了机构级自觉。**不是屎山，信噪比很高。**

**但 2026-07-29 的运行时审计给这份裁决打了一个折扣：写得好不等于跑得好。** Dagster 队列因并发槽泄漏死锁，夜间 ingest **连续 7 天零执行且无人察觉**；wechat / futu / gangtise 静默哑火 6.5 / 24 / 4 天，而运维台上的 cadence 戳仍是绿的；FMP 的 6 张表**有史以来零行**，却仍留在 `daily_enabled_sources` 里每晚空耗。这些都不是代码质量问题，而是**运行时可观测性的缺位**——已交付的东西没有人盯着它是否还在交付，而本文件此前把这些子系统一律记作"已落地"，读起来像它们都在健康运行。本轮据此新建了任务监控子系统（`src/xar/monitoring`，见"已落地"），并**反转了 P2-4"可观测性不自建"这条已记录的决策**。
...
6. **前端三模块重构（该轮头条之二；后经演进 7 新增 Andy 扩为四模块）**：同一 React SPA 由 FastAPI 单体托管，前端拆为三个并列 shell + 一套共享 ModuleNav 切换器（`web/src/components/ModuleNav.tsx`），全部坐在既有数据+本体底座上（无平行后端）：
   - **XAR Chathy**（默认首页 `/`，App.tsx:53）——ChatGPT 式、SSE 流式、**工具调用**分析师：不走 HTTP 回环，直接**在进程内**调用与仪表盘同源的函数（语义事实 / 混合文档检索 / 仪表盘 / 供应链图 / 公司·segment 详情 / data-room 文档，见 `src/xar/chathy/tools.py` 的 code-as-truth 工具注册表 + `execute()`）。后端：`models/llm.complete_stream()`（SSE 流式 + function-calling，复用任务管理器的 fallback/计费；`TaskClass.CHAT` 由 `settings.chat_models` 钉成精确有序链 kimi-k3-sub → glm-5.2-sub → minimax-m3-sub → deepseek-v4-pro，见 `RoutePolicy.chain_setting`；`chat_max_tokens=16000` + 满力度 `reasoning_effort`，因四席皆思考模型、小预算会被思考烧空 content）、`chathy/{tools,sessions,agent}.py`（`_MAX_ITERS=12` 轮工具循环 + 第 13 轮**收工回合**：撤掉 tools 逼模型用已取证据作答，撞上限也交答案不报错 agent.py:22/73;Postgres `chat_sessions`/`chat_messages` schema.sql:483-500）、`api/chathy.py`（SSE `POST /api/chathy/sessions/{sid}/chat` + 会话 CRUD）。前端 `pages/chathy/ChathyPage.tsx` + `components/chathy/*`（react-markdown、工具活动 chips、会话栏、fetch+ReadableStream 的 stop/abort）。
...
- **[已落地，但 FMP 一侧已死] Finnhub 公司新闻 ingestion（补真实源缺口）** —— `providers/finnhub.pull_news`(+`pull_general_news`)（finnhub.py:140/158）与 `providers/fmp.pull_news`（fmp.py:134）把公司新闻落进 `documents`（source='finnhub'/'fmp'、permission='grey'、仅摘要非全文、content-hash 去重）；`api/ops.py` 注册 `finnhub_news` 源（ops.py:146）+ `run_source` 分派（ops.py:259）；`kg/expert.ALT_SOURCES` 加入 finnhub/fmp（expert.py:29），使新闻同时进 build_kg 与 expert 层。**⚠️ 2026-07-29 更正：FMP 已被上游永久下线** —— v3/v4 **全部**端点返回 HTTP 403 `"Legacy Endpoint … no longer supported"`，**不是** 402（档位不足）也不是 429（配额用尽），因此**买 key、换档位、等次日恢复都无效**。它写的 6 张表**有史以来零行**，却仍留在 `daily_enabled_sources` 里，每分片每晚空耗约 45 分钟。**处置：从 `daily_enabled_sources` 摘除**；在摘除前，本仓库任何"FMP 可用 / 填 key 即点亮"的表述都是错的——这是唯一会让人去花钱买 key 的文档错误。Finnhub 一侧不受影响，仍是真实在产出的新闻源。
- **[已落地] 日度自动 ingest（曾列为待办，现真实部署）** —— `orchestration/daily.py:run_daily(stages=('pull','extract'))`：每源增量 PULL（按公司分片、隔离失败）→ parse/embed → build_kg → expert → signals → `resolve_forward_claims`（extract 阶段全局只跑一次而非每片；LLM 阶段预算上限封顶但廉价 DB 阶段照跑）。`storage/runlog.py` + 新 `ingest_runs` 表（schema.sql:443）= 运行日志 + 每源增量游标（last_success_ts）。CLI `xar daily`（cli.py:147）。content-hash + NOT-EXISTS 游标 → 幂等可续。**⚠️ 2026-07-29 更正：`ingest_runs` 已是死表** —— 最后一行停在 2026-07-22，87 行永久 `running`（无人收尾），唯一写入方 `orchestration/daily.py` 的那条路径**已不再运行**。真正全天在拉数的是 glmworker 自己的循环（`run_once` → `_pull_fresh` / `_backfill`），而它**零 runlog**。后果不只是"少了一张日志表"：它让"查 `ingest_runs` 就知道昨晚拉到没有"成了错误直觉，是 7 天零执行无人察觉的成因之一。**二选一，不要留假绿灯**：让真实拉取路径回写 runlog，或明确废弃该表。
- **[已落地] Dagster sidecar（日度运行时，现已部署）** —— `orchestration/definitions.py`：`pull_shard`（静态分片、06:00 调度，cron `0 {hour}`）+ `extract_all`（单 run、单批预算、06:30 调度，cron `30 {hour}`，**刻意不分片**因其读全局队列）+ `core_daily`（按需）。`docker-compose.yml` 新增 dagster 服务，host 端口 **3001**（UI/run-history/retries）+ `dagster_home` 卷；仅 app 容器跑 `xar init`（schema owner）。
  **⚠️ 它曾静默死锁 7 天（2026-07-29 审计发现、同日治理）** —— 此前 `DAGSTER_HOME` 下**根本没有 `dagster.yaml`**，一切走默认值，于是 `run_monitoring` 是关闭的。失效链：容器重建（`compose up -d` / 重启）打死正在跑的 run → 该 run 在 DB 里**永远停在 `STARTED`** → 每死一次泄漏一个并发槽 → 攒满默认 `max_concurrent_runs=10` 后**整个队列锁死**，此后每次 schedule tick 都只排队不执行。最坏的部分是它**看起来是健康的**：UI 上"有 run 在跑"、进程活着，只有产出是零。治理三项（配置落仓库版 `deploy/dagster/dagster.yaml`，由 compose **只读挂载**——放仓库而不是放卷，因为卷里的手改不受版本控制、换机即丢）：(1) **开 `run_monitoring` 且必须显式设 `max_runtime_seconds=28800`** —— `DefaultRunLauncher` 的 `supports_check_run_worker_health` 为 `False`，真正回收僵尸 run 的是 `check_run_timeout` 而非 worker 健康检查，只开 monitoring 不设这个超时等于没开；(2) **并发从单一全局闸改为分 job 限额**（`tag_concurrency_limits`：`pull_schedule=6` / `extract_schedule=1`，总闸 7）—— 单一全局闸下 pull 分片会把槽占满、把 extract 饿死；(3) 三个单步 asset job 改用 `in_process_executor`（省掉一层子进程，见 §七 内存实测），容器加 `init: true` 收僵尸进程。**2026-07-30 夜实测**：修复后 8 个分片全部启动（队列确已不再死锁），但 9 个 run 里有 4 个被 memcg OOM 杀——既验证死锁修复生效，也直接催生了"不提高 8G 限额、改从需求侧降"的内存决策（见 §七 ADR）。
- **[已落地] 任务监控子系统（2026-07-29 新建；补的是"已交付之后没人盯着它还在不在跑"这个洞）** —— `src/xar/monitoring/{catalog,detector,sweep,alerts,actions,control,dagster_gql}.py` + `src/xar/api/monitor.py`，暴露 `/api/ops/monitor{,/summary,/alerts,/history}` 与 ack / resolve / actions / mute；前端 `web/src/pages/ops/MonitorPage.tsx` 挂在 `/jarvy/monitor`（侧栏新增 "Monitor 任务监控"，带未解决 critical 的红点计数）；新增 `task_status_history` / `monitor_alerts` 两表。巡检是 **app 容器内的一个后台线程**（不引新服务、不引队列），每 120s 一轮，覆盖 **22 个任务**（常驻 worker + 13 个拉取源 + dagster + slx + 平台）。三条判定纪律，每条都是被上面那些故障逼出来的：(1) **双信号取较坏者** —— 心跳（这个任务尝试过没有）与产出（它真的产出了东西没有）分开采、判定取更坏的那个；7 天零执行那次心跳一路正常、只有产出是零，只看其一必然漏报；(2) **`Probe.degrade` 单独一路** —— 守护进程 unhealthy、队列死锁、部分 run 失败这类坏消息不属于"新鲜度"轴，硬塞进新鲜度会被时间窗抹平，故单开一个维度表达；(3) **恶化需连续 2 轮确认、恢复立即生效** —— 坏消息压抖动（避免一次网络抖动就推手机），好消息不拖延。告警走**专用 Telegram bot**（env `XAR_MONITOR_BOT` + `XAR_MONITOR_TELEGRAM_CHAT`，与业务 bot 分离，免得告警淹没在业务推送里）：critical 推手机、warn 只进页内；去重靠部分唯一索引 `(task_id) WHERE state<>'resolved'`。**带外兜底**：`deploy/monitor/deadman.sh` 已挂进主机 crontab 每 10 分钟——app 整个挂掉时进程内报警会跟着一起陪葬，这一层是唯一还能发声的。runbook：`deploy/monitor/README.md`。**起因记录在案免得重犯**：2026-07-29 审计发现 Dagster 队列死锁导致夜间 ingest 连续 7 天零执行无人察觉，且 wechat / futu / gangtise 静默哑火 6.5 / 24 / 4 天而 cadence 戳仍是绿的。该子系统**反转了 P2-4"可观测性不自建"的决策**，理由与代价记在 P2-4 条目下。 **2026-07-31 生产验证**:上线两天即逮到 `gangtise` —— cadence 戳 8 分钟前(每 12h 忠实尝试、`fn()` 不抛异常、于是次次盖绿戳),而最后一篇文档停在 07-25、**137.4 小时零产出**,判 `stale`(`worstBy=yield`)。**这是「只读心跳的检测器必然漏掉」的实证**,也是双信号取较坏者存在的唯一理由。同期配额治理亦获验证:finnhub 尾部份额 62%(上线前推算 62%,改前 38%),alphapai/aifinmarket 抽取率 100%、零积压。
...
- **工时**：1.5 天
- **⚠️ 2026-07-29：本条决策已被反转（原文保留在上，反转本身才是记录的重点）** —— 实际交付的是自建的任务监控子系统 `src/xar/monitoring`（见"已落地"条目）。
  - **为什么反转**：这里需要的不是通用 APM 关心的那三样（成本 / 延迟 / trace），而是**与本仓库任务语义强耦合**的判断，通用工具表达不出来：(1) **双信号**——心跳与产出取较坏者；7 天零执行那次进程一直活着、指标一路平稳，唯一异常是"该产出的东西没产出"，这在 APM 里根本没有对应的信号。(2) **cadence 戳绿而源已死**——wechat / futu / gangtise 哑火 6.5 / 24 / 4 天，戳记照常刷新；要判死必须先知道"这个源应该多久出一次东西"，那是领域知识，不是能靠通用阈值配出来的。(3) **非新鲜度类坏消息**（守护 unhealthy / 队列死锁 / 部分 run 失败）需要 `Probe.degrade` 单独一路承载。换句话说：**通用 APM 能告诉你进程还活着，答不了"这个任务是不是已经悄悄不干活了"**，而后者才是本仓库真正会亏钱的失效模式。
  - **代价（如实记账）**：多了一个自建子系统要长期维护（22 个任务的判定表 + 巡检线程 + 告警去重 + 带外 deadman）；以及"监控自己坏了谁来监控"这个绕不开的问题——只能靠 `deploy/monitor/deadman.sh` 那层主机 crontab 兜底，无法彻底消除。
  - **原决策仍成立的部分**：**没有**自建时序库、图表库或 trace 系统。监控页只是查 `task_status_history` / `monitor_alerts` 两张表，告警只推 Telegram。反转的是"不自建"，不是"可以造轮子"。
...
之后:    P2 按 monitor 触发（P2-4 已于 2026-07-29 反转为自建并交付，见该条目）
```
...
- ⚠️ 新增数据源需进运维控制台源注册表 + key-gated 优雅降级（arxiv/journals、新增 `finnhub_news`/fmp 公司新闻均已遵循：仅摘要非全文、content-hash 去重、permission='grey'）；非必要不再扩源。**⚠️ 2026-07-29：`fmp` 已被上游永久下线** —— v3/v4 全部端点返回 HTTP 403 `"Legacy Endpoint … no longer supported"`（非 402/429 配额问题，补 key 无效），其 6 张表有史以来零行，却仍在 `daily_enabled_sources` 里每分片每晚空耗约 45 分钟。**因此这条红线补一款：源死了要摘除**——留着的死源比没有源更糟，它同时留下"已配好源"的假象和真实的算力空耗。
...
- ❌ 日度 ingest 不引重型队列；✅ content-hash + NOT-EXISTS 游标做幂等可续，由 Dagster sidecar 调度（不侵入 app 容器，仅 app 跑 `xar init`）。**⚠️ 2026-07-29 更正（这条红线原先把一张死表写成了长期架构承诺，是本文件危害最大的一处失真）**：`ingest_runs` **已死** —— 最后一行 2026-07-22、87 行永久 `running`，唯一写入方 `orchestration/daily.py` 的路径已不运行；**真实拉取路径是 glmworker 自己的循环**（`run_once` → `_pull_fresh` / `_backfill`），零 runlog。红线本身（不引重型队列、靠 content-hash 幂等）依然成立，但**不要再把 `ingest_runs` 当作"昨晚拉到没有"的依据**——它是一盏坏掉的绿灯。要么让真实路径回写 runlog，要么明确废弃该表。
...
| 顶层模块 | 三模块并列：投研主轴 + Operations Console **Jarvy** `/jarvy`（11 个子页，含新增 monitor / fetchy）+ Exploration **Romy** `/romy`；前端为 Chathy `/` + Andy `/andy` + Genny `/genny` + Fenny `/fenny` 四 shell（原 Research Portal `/` → Genny `/genny`）；另有 `/phanny/*` | 全部复用同一 PG/LLM/embeddings 栈，分立 shell。**2026-07-11 更名**：`/ops/*` 与 `/explore/*` 保留为 302 兼容重定向而非直接删除，旧书签/旧文档链接不断；写新文档一律用 `/jarvy`、`/romy` |
...
| 日度 ingest 运行时 | Dagster sidecar（`pull_shard` 分片 06:00 + `extract_all` 单 run 06:30），host 端口 3001；**运行时配置走仓库版 `deploy/dagster/dagster.yaml` 只读挂载**（2026-07-29 补） | extract 全局只跑一次（不分片）；content-hash 去重 → 幂等可续；仅 app 容器跑 `xar init`。（原写"`ingest_runs` 游标"——该表已死，见 §六红线更正。）**此前 `DAGSTER_HOME` 下无 `dagster.yaml` → 全走默认 → `run_monitoring` 关闭**：容器重建打死的 run 永远停在 `STARTED`，每次泄漏一个并发槽，攒满默认 `max_concurrent_runs=10` 即全队列锁死——实际后果是**夜间 ingest 连续 7 天零执行且表面健康**。现开 `run_monitoring` 并**显式设 `max_runtime_seconds=28800`**（`DefaultRunLauncher.supports_check_run_worker_health=False`，回收僵尸靠 `check_run_timeout`，不设等于没开）；并发改分 job 限额 `tag_concurrency_limits`（pull=6 / extract=1，总闸 7）防 pull 饿死 extract；单步 asset job 用 `in_process_executor`；容器 `init: true` 收僵尸。配置放仓库而非卷：卷里的手改不受版本控制、换机即丢 |
| Dagster 内存上限 | **不提高 8G 容器限额，改从需求侧降**：单步 asset job 走 `in_process_executor` + `tag_concurrency_limits` 分 job 限额（pull=6 / extract=1） | 实测（容器硬限 8G）：固定开销约 580MB；worker import 基线 214MB；fastembed 模型 +618MB（仅 `parse_pending` 加载 ⇒ 只有 `extract_all` 付这笔）；`pull_shard` 约 730MB/run（in_process）vs 951MB（multiprocess）；`extract_all` 约 1759MB。**不提限的理由**：`docker.slice` 软闸 24G 已被打满、各容器限额之和已 22.5G，再提会把聚合推向 28G 硬闸——冲顶时受害的是整个栈，而本机是 7×24 交易机，为夜间批处理牺牲交易面不可接受。2026-07-30 夜实测 9 个 run 中 4 个被 memcg OOM 杀，正是这条决策的依据。⚠️ **复现性缺口**：8G 这个数字目前躺在**未被版本控制的** `docker-compose.override.yml` 里，换机即丢（且容器重建会把 `docker update` 的临时限额冲回 compose 值）——应比照 `dagster.yaml` 收进仓库 |　**2026-07-31 夜复测:决策成立但归因需修正** —— `memory.peak` 6.68/8 GiB(模型算 6.7G)、memcg OOM 0 次(对照 07-30 那晚 9 次);**但那一夜真正的约束是磁盘 IO 而非内存**(`nvme0n1` %util 174 / 831MB/s 读,内存尚余 18%)。故本决策的正确表述是:「不提高 8G」依然对,但**再上调并发先撞的是 IO,不是内存闸** —— 要提并发须先解决 IO(摘 FMP 每片约 45min 的 403 空转、或分片错峰),否则只会把 Postgres 拖成 unhealthy、连带饿死 qwendrain 的 `_claim()` 使 GPU 空转(实测约 30 分钟)。
