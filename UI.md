...
**五条平行产业链主题**：系统现覆盖 **5 条并行链 / 294 家公司 / 38 环节 / 25 条技术路线** —— 公司带
...
| `/ops/sources` | SourcesPage | 16 个数据源：可用性/姿态/行数/上次运行 + **一键运行** + **自检(selftest)** |
...
| `/ops/models` | ModelsPage | LLM vendor/model 路由(LiteLLM) + 定价 + 用量 + **Test LLM** |
...
| `GET /api/ops/llm` · `POST /api/ops/llm/test` | LLM | vendor/model 路由(LiteLLM)、定价、`llm_usage` 用量；test=真实廉价往返 |
...
**数据源注册表（16 源，6 类）**：filing（edgar/cninfo）· web（news/jobs/wechat）· market（yahoo/finnhub/fmp/polygon/wind/aifinmarket）·
...
**专家智能体层**（`kg/expert.py`）：对另类数据（X/公众号/news/AIFINmarket）做 LLM 相关性/立场/质量过滤
→ `expert_insights` 表 + `kg_events(license=expert)`，信噪比放大在 `/ops/altdata` 展示。

**LLM 路由**（`config.py`）：**DeepSeek V4 为默认** —— `XAR_MODEL_FAST=deepseek/deepseek-v4-flash`（抽取/分类/快速综合）、
`XAR_MODEL_STRONG=deepseek/deepseek-v4-pro`（推理/辩论/前沿综合）、`XAR_MODEL_EFFORT=high`；可经 LiteLLM 覆盖为任意模型
（如 `claude-opus-4-8` / `claude-haiku-4-5`，配 `ANTHROPIC_API_KEY`）。`complete_json` 走 JSON-mode；V4 推理模型按 `reasoning_effort` 调档；每次运行有 USD 预算上限。
