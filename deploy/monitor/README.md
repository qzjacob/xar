...
## 六·五、夜检脚本 `verify_nightly.sh` 的四个坑(2026-07-31 修,勿回退)

这四处曾让脚本对 2026-07-31 那一夜给出**方向相反**的结论:实际是「三项治理全部生效、
dagster 零 OOM」,脚本却报「5 次 memcg OOM、内存治理未达标」。修完才看清真因是
容器 04:33 重启打死了全部 run worker。**改这个脚本前先读这一节。**

1. **OOM 必须按 cgroup 归因,不能按主机汇总。**
   原来是 `journalctl | grep -c 'Memory cgroup out of memory'` —— 把 glmworker 已知的
   5G 自愈循环(每 3-4h 一次)算到了 dagster 头上。现在解析每条 oom-kill 行的
   `oom_memcg=/docker.slice/docker-<id>`,只认 dagster 自己的 cgroup;
   查不到名字的如实标「已销毁(容器被重建过)」,不猜。
   判据:**被监控对象的健康,只能用它自己 cgroup 的数据判。**

2. **`memory.peak` 在容器重启后归零 —— 重启过就是「测不到」,不是「通过」。**
   原来无条件把它当夜跑峰值读,重启后读到 0.73G 还打 ✓「未逼近 8G」。
   **把「测不到」报成「通过」比报失败更糟**:它让人以为治理已验证。
   现在先比对 `StartedAt` 是否落在核验窗内,落在窗内就判不可验证,并说明原因。

3. **在飞的 run 没有 `end_time`,峰值并发不能写成 `(s, e or s)`。**
   零长区间 ⇒ 「全部 run 都还在飞」的那一夜峰值并发算出来恒为 0,脚本还据此打 ✓
   「并发上限生效」。未结束 = 一直占着槽,必须按 `now` 收尾。
   端点排序还要让同一时刻的 -1(结束)排在 +1(开始)前,否则首尾相接会算出虚假重叠。

4. **时间戳一律带时区、且 `read` 的字段里不能有空格。**
   原来直接打印 RFC3339 的 UTC 串,与本地时区的表头并排,出现过
   「容器启于 08:33 而现在 08:15」的时间倒流 —— 正是这个矛盾提示了结论不可信。
   修时又踩了第二个:`local.strftime('%m-%d %H:%M:%S%z')` 里的空格把
   `read -r A B C D E` 的字段整体挤位,`DRESTARTED` 拿到时间串而非 yes/no,
   「窗内重启」分支永远不触发。改用 `%m-%dT%H:%M:%S%z`。

另补了一条**僵尸 run 判据**(2026-07-31 那次误判的关键):
`DefaultRunLauncher` 的 run worker 是 dagster 容器内的子进程,所以
「状态 STARTED 但容器里零 run worker」= run 已经死了、只是没人改状态,
它们会一直占着并发槽直到 `run_monitoring` 超时回收。有这一条才能一眼分开
**「跑失败了」和「跑的人被杀了」** —— 这两者的处置完全不同。
⚠️ 探针必须用 `os.getpid()` 排除自己:它自己的命令行里就含要找的那个字符串,
不排除就永远至少数出 1 个,判据直接失效。

⚠️ 还有一个通用坑:**`cmd | python3 - <<'PY'` 不成立** —— heredoc 本身就是 stdin,
会盖掉管道,`sys.stdin` 读到的是脚本自己。要么让 python 自己调 `subprocess`,
