...
def sync_metric_prints(as_of: date) -> dict:
    ...
    with connect() as conn:
        ...
        for metric_key, name_zh, hardness, unit in rows:
            ...
            # 发射节流（AM）：日频市场序列（美债/OAS/VIX…）每个新印字都发=工人日跑日刷屏。
            # bridge_min_gap_days>0 时,距该指标最近一条 macro_print 不足间隔即跳过
            # （dedup 只防重不防频,这道闸防的是"频"）。
            if link.bridge_min_gap_days:
                last = db.query(
                    "SELECT max(event_date) d FROM kg_events "
                    "WHERE event_type='macro_print' AND attrs->>'metric_key' = %s",
                    (metric_key,))
                # slx observation.valid_time 是 timestamptz,kg_events.event_date 是 date
                vt_d = valid_time.date() if isinstance(valid_time, datetime) else valid_time
                if (last and last[0]["d"] is not None
                        and (vt_d - last[0]["d"]).days < link.bridge_min_gap_days):
                    skipped += 1
                    continue
            try:
