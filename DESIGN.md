...
**同周期排序不变量**：`glm_worker._llm_stage` 在 `llm.pinned(pin)` 内**先 `triage_pending` 后 `build_kg`**——同一轮新拉进来的微信文档必须先打分，才能被本轮的深抽守卫正确门控。**订阅纪律**：triage 走新 `TaskClass.WECHAT_TRIAGE`（`CHEAP_BULK` + `SUBSCRIPTION` 优先，链外**永不回退计量 token**，见 §6.1），与夜间批量抽取共享同一钉扎链——**2026-07-19 起链首为本地 `qwen3-14b-local`（零成本），云 GLM 订阅池降为回落**（额度进一步释放）。
...
### 5.13 CN 非标投研数据专属编排(As-Built,2026-07,`ontology/research_docs.py` + `providers/gangtise/{insight,planner}.py` + `providers/alt/wind_edb.py` + `orchestration/research_audit.py`)

**意图**:把 Gangtise / Wind 的**非标准化语义数据**——券商研报 / 会议·业绩会纪要 / 专家纪要 / 经营讨论 MD&A / 数据追踪(EDB 指标)——建成**专属本体编排**,持续供给 §5.12 的论点撰写与追踪(dossier→evidence_link→争论天平),而非停在结构化财务/行情。GLM 常驻工人自动抓取存档:**核心公司优先→非核心、最新优先→历史前溯、每日刷新**;另建**独立智能体**每日复核抓取质量。四层增量,零新表(documents.meta + kvstate 水位线足够)。

- **文档类型本体(`ontology/research_docs.py`,代码即真相)**:`ResearchDocSpec` 注册表(doc_type/vendor/endpoint/kb_resource_type/**extraction 道**(expert/kg_only/none)/rating_extractor/典型 catalyst_types + pillar_kinds/kg_priority/cadence SLO/body),7 类(broker_report/meeting_minutes/expert_minutes/mgmt_discussion + 既有 one_pager/investment_logic/peer_comparison)。`kg_priority_case()` 编译可信常量 SQL CASE(同 `source_priority_sql` 手法)。前置:`ingestion/base.Doc.doc_id` 加性字段——来源天然主键(summaryId/reportId)让 brief→更新原地落同一行,不裂哈希。
- **抓取器(`providers/gangtise/insight.py`,零 LLM,保守只存摘要)**:`pull_broker_reports`/`pull_minutes`(**全局日期窗扫描**,一页 50 行覆盖全市场,`securityList` 数字段+交易所双匹配反解 registry,防 9999.SS↔09999.HK 撞车)、`pull_mgmt_discussion`(from-earningsCall/announcement,按 reportDate 取历史季度)、`pull_clues`(security_clue 变更雷达,**不落库**只出目标集)、`parse_broker_ratings`(零 LLM 确定性评级第二遍,镜像 `cninfo.parse_research_ratings` → `analyst_ratings(source='gangtise')`)。text=title+brief+essence[];**不碰任何文件下载端点**(零信用消耗)。
- **数据追踪(`providers/alt/wind_edb.py` + `altdata.py`)**:每主题 3-6 条策展 `AltSignalSpec(source='wind_edb', scope='theme')`,固定中文 question(`EDB_QUESTIONS` code-as-truth)→ Wind `economic_data.natural_language_get_edb_data` → `alt_signals`(PIT)→ 既有 `sync_alt_events`(|z|≥2→kg_events)→ `thesis_signals` 支柱校正,**零下游改动**。逐指标单位/日期容错,单指标失败不拖批。
- **语义路由(最小接线)**:`kg/expert.py ALT_SOURCES += 'gangtise'`——研报/纪要经 expert 语义道 → `semantic_facts` → dossier「语义事实」+ `evidence_link` 相对主张分类(下游全自动);`_SYSTEM_RESEARCH` 提示词变体(策展专业内容非噪声:抽报告自身论断与**预期差**,stance 取报告观点,实体解析失败落锚公司 fallback);`kg/extract.build_kg` ORDER 注入 `kg_priority_case()`(研报升至 1,与 cninfo/news 平级)。
- **抓取规划器(`providers/gangtise/planner.py`)**:`cn_priority_order()`(种子∩CN→覆盖度 composite→注册表序);`fresh_sweep()`(每日 24h 节拍:clue 雷达 + 全局日期窗扫 + 核心 MD&A + 评级);`backfill_step()`((doc_type,月窗) 单元最新月先行,**连续 2 空窗盖 exhausted 戳**对付试用账户 ~1 月历史上限,MD&A 季度序不受限,毒单元 retry-once-then-skip,游标 crash-safe)。折入 glm_worker `_pull_fresh` 四站(gangtise_insight/gangtise_backfill/wind_edb/aifinmarket_theme,后者修 `pull_theme_news` 孤儿),`_gangtise` 轮转序换核心优先序。
- **独立审计(`orchestration/research_audit.py`)**:`integrity_report()`(零 LLM:各 doc_type 计数/24h 增量/链接率/KG 积压/expert 覆盖/评级/EDB 新鲜度/clue 对账)+ `spot_check()`(**`TaskClass.AUDIT`→STRONG/TOKEN,永不落 GLM 订阅池**,`_grounded` 零 LLM 预检后逐篇裁决)→ `run_audit()`(失败裁决 → `kg_extracted_at=NULL` 重排队 + meta.audit 标记)。glm_worker `_due("research_audit",24h)` 在 **pinned 与 quota 门之外**调用(验收模型 ≠ 生产模型、GLM 耗尽不影响审计——独立性两半)。
- **表面**:CLI `xar research {crawl,backfill[--reset],audit[--no-llm],status}` · `GET /api/ops/research-crawl`。**Schema 零 DDL**(documents/analyst_ratings/alt_signals 复用;kvstate `gangtise_crawl`/`gangtise_backfill`/`gangtise_clue_state`/`wind_edb_state`/`research_audit`)。真机纪律:Gangtise 字段名以真机为准修正离线 fixture(前科:资产负债表字段位错)。

---

### 5.14 围绕季报的 360° 事件交易投研编排(As-Built,2026-07,`ontology/earnings_events.py` + `research/earnings.py` + `providers/alt/implied_move.py`)

**意图**:把平台从"论点跟踪"推进到"**事件交易决策支持**"——对临近季报的**美股**标的,组装 360° dossier(指引惯例/一致预期与修订/beat-and-raise 习惯/数据追踪/**期权隐含波动**/情绪/估值),由 LLM 判官输出**做多/做空/不交易 + conviction 0-10(≥7 可操作)**,并做**盘后回验**攒战绩校准。ServiceNow 式画像:指引+预期+惯例+期权隐含波动+估值 → 「做多@7」。仅美股(期权链/一致预期 yfinance 免费可得);唯一新表 `earnings_verdicts`,其余全部复用既有基础设施。

- **本体(`ontology/earnings_events.py`,代码即真相)**:`EarningsVerdict` pydantic(direction/conviction 0-10/8 维 `DimensionRead`/expected_surprise/move_view/plan/falsifiers/asymmetry)兼作 `llm.complete_json` schema 与入库 content;`validate_verdict` 五规则的**证据密度门**(≥7 需 ≥6 去重接地锚 + asymmetry 非空 + direction≠no_trade;no_trade→conviction=0;幻觉 id 拒),宁缺毋滥,与 `validate_thesis` 同哲学。`EARNINGS_UNIVERSE` ~31 策展美股旗舰(∩ registry,测试锁 US ticker)。conviction 0-10 事件域与 `CompanyThesis` 1-5 论点域**互不换算**。
- **数据获取(零 LLM)**:`providers/alt/implied_move.py` —— 观察窗内 universe 名字的 **ATM straddle 快照**(yfinance 免费主路;`massive_api_key` armed 时切 fcn 真 IV)→ `alt.options_implied_move`。**关键 PIT 裁决:`period_end`=快照日**(IV 是当日可观察量,语义正确且不撞 `alt_signals` 唯一键),`meta.earnings_date` 是事件锚 → dossier 按它取本事件 IV run-up 序列。走 `AltSignalSpec` + `AltBinding.options_ticker` 派生 → z-score/支柱芯片/`sync_alt_events` **零下游改动**(good_when=None → 纯注意力旗标,不参与支柱评分)。`research/earnings.py` 批一:`reaction_return`(amc→close(D+1)/close(D)、bmo→close(D)/close(D-1))、`beat_stats`、`hist_move_stats`、`refresh_window`(轮转游标全 universe `yahoo.pull_calendar` + 窗内 analyst/implied 快照)。顺带修 `structured.upsert_calendar` 的 meta 整体覆盖 bug → **jsonb 合并**(保 finnhub `hour`/surprise,加性全源受益);`yahoo.pull_calendar` 增写 `meta.session`(amc/bmo)。
- **dossier(`dossier_earnings`,零 LLM)**:11 节接地面板(事件头/预期设定+90 天修订漂移/beat 习惯/guidance 习惯+兑现率/评级动量+PT vs 现价/implied vs 历史/情绪 14d/alt 快照/论点状态 health_v3+debates/价格语境/覆盖缺口),每节 try/except 单节失败不沉整包;id 复用 `[estimate:] [event:]` + 新 `[calendar:] [alt:] [ratings:] [price:]`,validator 只做 known_ids 精确匹配(禁幻觉)。
- **裁决引擎(`build_verdict`/`judge_due`)**:`TaskClass.EARNINGS_JUDGE=RoutePolicy(STRONG, TOKEN)`;host 上 `_preferred_pin` 择优 **CODEX_PIN/CLAUDE_MAX_PIN**(深度研究订阅执行器,$0),docker/无执行器落 deepseek 强 token;`earnings_verdict_host_only` 开关可令 docker 返回 `deferred_host`、host 专跑。dossier→complete_json→validate→带违规清单重试一次→仍违规**拒绝入库**。**INSERT 即锁**(`earnings_verdicts`,无 locked_at,`created_at` 即锁定时刻);已有裁决且非 force→skipped,`--force`→version+1;T-3 后数据剧变**不自动重生成**——面板算"锁后漂移 chips"展示陈旧度但保校准诚实。
- **闭环 + 编排**:`score_outcomes`(未评分且过财报日→±3 天对齐 occurred→实际 surprise + `reaction_return` + realized-vs-implied + direction_hit;no_trade=abstain;超期→`event_moved`/`price_missing` 收尾**绝不无限挂起**)、`calibration`(conviction 分桶 × 命中率 × 平均反应,≥7 桶单列)。`glm_worker._pull_fresh` 加 `_run("earnings_watch",6h,refresh_window)`;`run_once` 加模块级 `_earnings_step`(**GLM pin/quota 门之外**,`_research_audit_step` 同款可打桩:judge_due 24h + score_outcomes 12h)。`xar earnings watch/panel/judge/outcomes/calibration`;`GET /api/ops/earnings` + `POST /api/ops/earnings/{cid}/judge`;`dashboard._earnings_block` → 公司页 `EarningsSection.tsx`(方向+conviction 表针 ≥7 高亮/implied vs beat/倒计时/锁后漂移/outcome 战绩)。

### 5.15 统一能力架构(UA,As-Built,2026-07,`capabilities/{registry,runs}.py` + `macro/view.py` + `/api/run/*`)

**意图**:消除"同一分析能力四处重复接线"(thesis build 曾有 4 入口、earnings judge 3 入口、**4 种互不相同的异步触发风格 + 5 个状态存储**),并把全模块(Chathy 对话 ↔ Genny 研究页 ↔ Andy 宏观 ↔ ET 季报事件 ↔ Explorer ↔ Fenny ↔ 报告)耦合成**一处定义、多入口共用**:不管从 Chathy 对话还是 Genny 页面进来,都进入同一全体系分析架构。

- **能力登记簿(`capabilities/registry.py`,代码即真相)**:`CapabilitySpec{name, description, parameters(JSON-schema), fn(进程内普通函数), kind: read|build, duration: fast|slow, chathy: bool}`。由 `chathy/tools.py` 的 24 个 ToolSpec **原样泛化迁入**(tools.py 退化为 re-export shim,agent.py/测试零改动);`chathy=True` 的经 `openai_tool_defs()` 渲染成 Chathy 工具,`execute()` 跑并 8k JSON-安全截断。新增能力 = 追加一条 spec。
- **统一异步触发(`capabilities/runs.py` + `capability_runs` 表)**:`build/slow` 能力全走一条道——`schedule(name,args,origin)`(>30min 陈旧 running 先收割 → 活跃去重命中返回既有 run_id → INSERT queued,并发撞 `uq_capruns_active` 部分唯一索引读回既有行)+ `execute_run(run_id)`(queued→running→fn→done+result / error,**绝不 raise**)。`GET /api/capabilities`、`POST /api/run/{name}`(read/fast 内联返回 result;build/slow schedule+BackgroundTasks → run_id)、`GET /api/run/{run_id}`、`GET /api/runs`;`xar run <name> --args JSON` + `xar capabilities`。`POST /api/ops/earnings/{cid}/judge` shim 化(经 build_earnings_verdict 能力,返回 run_id,活跃去重防 worker/UI 双跑——叠加 build_verdict 的 INSERT 锁 = 双保险)。
- **ET 三向耦合**:新 `macro/view.py`(research 层,禁 import ..api;惰性→andy_links 无循环)——`theme_macro_view`(link_theme 活读数 PIT;slx 不可用降级 static rationale,live=False)/ `compact_theme_macro`(dossier·Chathy·报告**共享 8k 压缩器**:drop series 保水印)/ `macro_dossier_lines`(`[registry:macro:*]` 接地行)。`earnings.dossier_earnings` 加 §8b 宏观勾稽(Andy 活读数)+ §8c 主题争论天平(`theme_debate_health` 旗舰聚合)两节;`thesis.dossier` 宏观节升级为同一活读数。
- **Chathy 读+触发**:7 个新工具——`earnings_panel`/`earnings_verdict`(refresh=true → schedule 返 run_id,不卡 SSE)/`run_status`/`theme_debates`/`exploration_frontier`/`fenny_quote`(进程内 fcn)/`start_report`(schedule report 能力)。慢动作一律 schedule + run_id + "勿轮询"提示。
...
- **`models/registry.py`——code-as-truth 模型库（可更新的"模型库"）**：`Provider` + `ModelSpec` 数据类；枚举 `Billing`(token|subscription) / `Capability`(fast|strong|reasoning|long_context|cheap_bulk) / `Status`(active|preview|deprecated)。`PROVIDERS`：deepseek、anthropic、openai、**zhipu(=GLM)**、**moonshot(=Kimi)**、**ollama(=minis RTX 3090 本地,2026-07-19)**。`ModelSpec` 另增 `executor` 字段（`litellm` 默认 | `agent_sdk`）。`MODELS`：token 模型（DeepSeek v4-flash/pro、Claude opus/haiku/sonnet）+ GLM/Kimi **订阅**条目 + **Claude Max 订阅**条目（`claude-opus-max`/`claude-sonnet-max`，`executor="agent_sdk"`，见下）。辅助 `candidates_for(capability, billing_pref)`（billing 优先稳定排序、保留 token 回退尾）、`preferred`、`get`/`by_litellm`/`provider_of`。**换代 = 改这一个文件**（加 `ModelSpec`、置 `preferred=True`、旧的翻 `deprecated`）；`_PRICES` 从 `MODELS` 派生。
- **`models/router.py`——任务路由**：`TaskClass`（11 类：kg_extract、expert、search_bulk、analyst、debate、editor、judge、synth、eval、adhoc_fast、adhoc_strong）+ `RoutePolicy` + `POLICIES`；`resolve(task)` 返回有序候选链。bulk/search 任务 → `CHEAP_BULK` + 订阅优先（GLM/Kimi 平价，使对 947 司语料的夜间抽取**永不跑出无界 token 账单**），再接预算内的廉价 DeepSeek token；quality 任务（debate/editor/synth）→ 强 token + 跨厂回退。**2026-07-19 起 glmworker 钉扎路径更进一步：本地优先**（`_fetchy_pin` 前插本地头，见下「本地 ollama 条目」；云 GLM 订阅降为自动回落），通用路由链不变。解析优先级：`route_overrides` 表（ops API）> env（`XAR_MODEL_*`）> 注册表 `preferred`。`tier="fast|strong"` 作向后兼容别名（`as_task`），未迁移调用点不变。
...
- **`models/agentsdk.py` + `_agentsdk_worker.py`——Claude Max 订阅执行器（第二执行路径，2026-07）**：与 litellm（HTTP、按 token 计费）并列的第二条执行路径，`ModelSpec.executor=="agent_sdk"` 的两个订阅条目走此路——`claude-opus-max`(STRONG/REASONING/LONG_CONTEXT) / `claude-sonnet-max`(STRONG/FAST)，`Billing.SUBSCRIPTION`、0/0 价、`litellm_model="anthropic-max/…"`（与 token 版 `anthropic/claude-*` 索引不撞车；`agentsdk` 从 `/` 之后取真实模型 id，`claude-opus-max` 另受 `anthropic_max_model` 覆盖）。它们经 **Claude Agent SDK** 复用 Claude Code 的 OAuth 凭证（`~/.claude/.credentials.json` = **Max 订阅**登录）跑**单次补全**（`allowed_tools=[]`/`max_turns=1`/`permission_mode="bypassPermissions"`/`setting_sources=[]`——非 agent loop），**零 token 计费**——与 GLM 订阅池（`glm_worker`）同一「订阅内白嫖、订阅外分文不花」纪律。**订阅强制**：在**剥掉 `ANTHROPIC_API_KEY` 的子进程**里**按文件路径**调用 worker（而非 `-m …`，否则导入 xar 会把 `.env` 里的 key 经 `_ensure_keys` 重新灌回 env、盖过订阅登录；worker 零 xar 依赖），父进程 `os.environ` 不动、与 app 侧并发的计量 Anthropic 调用互不干扰。`available()` 闸：`anthropic_max_enabled` + `claude_agent_sdk` 可导入 + `claude` CLI 在 PATH + 凭证在位 → **仅宿主机**；docker 容器缺 CLI/凭证即 `False`，`llm` 据此跳过、回退 GLM/DeepSeek。`is_quota_error` 与 `glm_worker` 同形（Max 有 5h 窗/周额度，钉扎链下额度耗尽=等待而非落 token）。**路由——peer/fallback，非默认 lead**：`llm.complete()` 见 `executor=="agent_sdk"` 走 `agentsdk.complete`（不可用即轮转下一候选），记 `used_sub=True`→`usd=0`（订阅、不触预算闸）；Claude-Max 只进 **STRONG 链（editor/synth/debate）作 token+GLM lead 之后**的候选（`prefer_billing=TOKEN` 保 token 优先），**缺席批量**（KG_EXTRACT/EXPERT/WECHAT_TRIAGE 仍钉 GLM——~6.5s/次 + 订阅额度，不适合千级文档）；`complete_stream()` 跳过（单次、不支持工具流）。便捷钉扎链 `CLAUDE_MAX_PIN=("claude-opus-max","glm-5.2-sub","glm-4.6-sub")` 强制高价值流程走 Opus、订阅/宿主不可用则优雅降级到 GLM（不改全局默认路由）。`config.py` 增 `anthropic_max_enabled`(默认 True)/`anthropic_max_model`(claude-opus-4-8)/`anthropic_max_effort`(high)/`anthropic_max_timeout_s`(180)；`pyproject` 增 `[agentsdk]` extra（`claude-agent-sdk>=0.2`）+ `.env.example` 块；`/api/ops/llm` 增 `anthropicMax` 块（enabled/available/model/effort）；**部署仅宿主侧**（凭证不进 docker）；`tests/test_agentsdk.py`（10 项离线）。
...
- [x] **glmworker 本地化（Phase 3+4，2026-07-19）**：本地优先钉扎 + 赛马换代 `qwen3-14b-local`（见 §6.1 本地 ollama 条目）；生产 6h 实证云端占比 0%。
- [ ] **本地 LLM 后续三项**（详见 `LOCAL_LLM_PLAN.md`）：① glmworker 容器 ~3-4h anon 长到 5G mem_limit 被 memcg 杀的既往循环——根因排查与修复；② 7 天 soak（至 2026-07-26）通过后的收尾清理——败者 spec/tag 删除、`glm4-local` 退役；③ ml.slice 页缓存顶 High 的观察项——冷加载劣化则执行 `--big` 二切 12G/16G。
