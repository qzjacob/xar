...
| **模型** | **LiteLLM LLM 任务管理器**（按任务路由 · 多供应商 DeepSeek/GLM/Kimi/Anthropic/本地 ollama · token-vs-订阅-vs-本地零成本计费感知）+ 成本追踪 + 预算上限 | 编辑 `models/registry.py` 一处即换代 / 任意 LiteLLM provider |
...
- **本体富化已覆盖全部 947 家至完整深度**：基础本体（sector/industry/segment/chain_role）原本就 100% 完整；`scripts/ontology_enrich.py`（白名单校验的批量 LLM 富化，经任务管理器以 `task="search_bulk"` 路由到 GLM 订阅、DeepSeek 回退，528 家约 $0.43）为 569 家批量生成的"宇宙"公司补足**深度维度**：多主题成员、技术路线暴露、更丰富别名、精炼主段位 —— 一切严格对本体词表校验，越界项丢弃；**技术路线更经一条 code-as-truth 跨域不变量校验**（`registry.ROUTE_THEMES`：33 条技术路线各自声明"归属主题"，`_valid()` 在富化**源头拒绝**任一归属主题与公司主题零交集的越域路线 —— 如芯片公司被打上太空推进路线 —— 把可规则化的"供应商↔路线混淆"错误从事后补丁表**上提为源头不变量**，重跑富化不再重生该类错误；交集即放行的宽松设计，如 `tr_cv`(计算机视觉) 对 ai_software / humanoid / ai_chip 三主题均合法）；一张确定性 `_CORRECTIONS` 表编码 18 处审计确认修正，`generate()` 合并缓存+修正后重写 `universe.py`。富化后的 `tech_routes` 经 `kg/store.py` 的 `bootstrap_seed` 落为 `uses_techroute` 边（`license_tag='enriched'`），重播种时"删后重建"以让修正干净传播。**结果（全库 947）**：多主题公司 80，技术路线节点 33，`uses_techroute` 边 724（360 为 enriched），`competes_in` 1024，`entity_aliases` 3623；经独立双重审计 + 代码审查，词表 0 违规、5 项完整性不变量通过。
...
| **X 专家之声** | `providers/twitter.py`（**仅精选研究者 handle**，回复过滤；**默认关 + $20/月总限额**，fail-closed 记账） | 🟡 灰（自用事实摘录） | 真正的前沿研究者声音（非噪声关键词检索），落 `documents(source='x')` |
...
| 8 | **人工审批** | `interrupt()` 风格，报告默认 `awaiting_approval`，POST `/api/report/{id}/approve` 才发布；**状态闸**：仅 `awaiting_approval` 可批准（published 幂等/running·failed 拒绝），异常统一落 `failed` | 强制非投资建议免责声明 |
...
- **可更新模型库（code-as-truth）`registry.py`**：`Provider` + `ModelSpec` 数据类，枚举 `Billing`(token|subscription) / `Capability`(fast|strong|reasoning|long_context|cheap_bulk) / `Status`。供应商覆盖 **DeepSeek · Anthropic · OpenAI · GLM(Zhipu) · Kimi(Moonshot) · ollama(本地 RTX 3090)**；`MODELS` 同时收录 token 计费模型、GLM/Kimi 的**订阅(subscription)条目**与**本地零成本条目**（现役 `qwen3-14b-local`，2026-07-19 赛马选定；`capabilities=()` 仅钉扎防越权入通用链）。`candidates_for()` 按计费优先稳定排序产出候选链，`_PRICES` 从 `MODELS` 派生。**换代 = 编辑这一个文件**（加 `ModelSpec`、置 `preferred=True`、把旧的翻成 deprecated）。
- **任务路由器 `router.py`**：`TaskClass` 枚举 11 类（`kg_extract`/`expert`/`search_bulk`/`analyst`/`debate`/`editor`/`judge`/`synth`/`eval`/`adhoc_fast`/`adhoc_strong`），`POLICIES` + `resolve(task)` → 有序候选链。**批量/搜索类**（kg_extract/expert/search_bulk）→ `CHEAP_BULK` + **订阅优先**（GLM/Kimi 包月，于是对 947 家公司的夜间抽取走**固定费率**而非无上限 token 账单），再退到预算内的廉价 DeepSeek token——**且 glmworker 抽取钉扎链 2026-07-19 起本地优先**（RTX 3090 ollama 零成本，云订阅降为自动回落）；**质量类**（debate/editor/synth）→ STRONG token + 跨供应商回退。解析优先级：`route_overrides` 表（ops API）> env(`XAR_MODEL_*`) > registry `preferred`；`tier="fast|strong"` 经 `as_task` 保留为向后兼容别名，未迁移的调用点零改动。
...
**它真的抓到了东西**：上线两天，监控逮到一个投研源已经**静默 5.7 天没有产出任何内容** ——它每 12 小时都在按时尝试、每次都不报错，所以任何只看"跑没跑"的看板都会显示它一切正常。双信号把"尝试过"和"真的拿回东西了"分开看，才让它现形。
...
#   OLLAMA_API_KEY=ollama            可选：glmworker 抽取走本地 ollama（零成本，云 GLM 回落）
